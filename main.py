# main.py — Streamlit App for Structured Research Repository and Querying
...
# ----------------- Tabs Layout -----------------
tab1, tab2 = st.tabs(["📄 Upload & Summarize Paper", "🌐 Search Open-Access Papers"])
...
with tab1:
    ...
    if uploaded_file:
        ...
        if text.strip():
            with st.spinner("🧠 Summarizing paper using AI..."):
                structured_summary = summarize_paper_structured(text, uploaded_file.name)
    ...
    if st.session_state.repository:
        ...
        query = st.text_input("Ask a question about your uploaded papers:")
...
with tab2:
    ...
    user_prompt = st.text_input(
        "Enter a topic or research question (e.g., 'AI ethics in healthcare'):",
    ...
    if st.button("Search Papers"):
        if not user_prompt:
            ...
            with st.spinner("🔎 Searching open-access repositories..."):
                try:
                    summary, insights_df, references = search_open_access_papers(user_prompt)
            ...
            if not insights_df.empty:
                ...
                with st.expander("📖 View Reference Links"):
                    for i, ref in enumerate(references, 1):
                        st.markdown(f"{i}. [Paper Link]({ref})")
            else:
                st.info("No papers found for your query. Try a different search term.")

# ---------------------------------------------------------------------
# 🔹 Footer
# ---------------------------------------------------------------------
st.markdown("---")
st.caption("Developed for AI-enhanced academic research exploration.")
